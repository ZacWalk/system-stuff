...
BOOL CTabWnd::Create(DWORD dwStyle, CWnd* pParentWnd, UINT nID)
{
        CRect rect, parent;
        ASSERT(pParentWnd);
    if (!CWnd::Create(NULL, "_TabWnd", WS_CHILD|WS_VISIBLE, rect, pParentWnd, nID, NULL))
        return FALSE;
...
BOOL CTabWnd::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
  if (m_selTab!=-1)
  {
      *pResult = m_viewList.GetAt(m_viewList.FindIndex(m_selTab))->pWnd->SendMessage(WM_NOTIFY, wParam, lParam);
  }
